...
class Model
{
    ...
    public function __call($method, $args)
    {
        if (in_array(strtolower($method), $this->methods, true)) {
            ...
            // 统计查询的实现
            // $field =  isset($args[0])?$args[0]:$this->pk; 更新了字段获取机制 邹广圆 20170302
            if (isset($args[0])) {
                $field = $args[0];
            } elseif (in_array($this->pk, $this->fields)) {
                $field = $this->pk;
            } else {
                $field = $this->fields[0];
            }
    ...
    protected function _parseOptions($options = array())
    {
        ...
        if (!isset($options['table'])) {
            ...
            $fields = $options['field'] ? $options['field'] : $this->fields;
        } else {
        ...
        // 字段
        if (is_null($this->options['field']) && is_null($options['field'])) {
            $options['field'] = $this->fields;
        }
        ...
        if (isset($options['where']) && is_array($options['where']) && !empty($fields) && !isset($options['join'])) {
            ...
            foreach ($options['where'] as $key => $val) {
                $key = trim($key);
                if (in_array($key, $fields, true) || in_array($key, $this->fields, true)) {
                    if (is_scalar($val)) {
